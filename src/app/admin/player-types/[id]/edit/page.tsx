...
export default function EditPlayerTypePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        const res = await fetch(`/api/player-types/${id}`)
        if (!res.ok) {
          setError('Player type not found')
          setLoading(false)
          return
        }
        ...
        setError('Failed to load player type')
      } finally {
        setLoading(false)
      }
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/player-types/${playerTypeId}`, {
        method: 'PUT',
      ...
      if (!res.ok) {
        const data = await res.json()
        setError(data.error || 'Failed to update player type')
        return
      }
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Edit Player Type</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-purple-500 focus:border-purple-500 outline-none transition-colors text-gray-900"
                placeholder="How to set up your device"
              />
            </div>
            <div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            {saving ? 'Saving...' : 'Update Player Type'}
          </button>
          <Link
            href="/admin/player-types"

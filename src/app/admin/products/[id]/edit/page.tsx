...
import { ArrowLeft, Plus, Trash2, Save, Upload, X, Star, GripVertical } from 'lucide-react'
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        setOptions(
          (data.options || []).map((opt: { name: string; price: string | number; screens?: number; duration?: number; popular?: boolean; sortOrder: number }) => ({
            name: opt.name,
